...
## Crowd arc: curved stand wall + animated shader + the gameplay intensity hook.
## Split out of main.gd (audit §4.1) — pure cut/paste, zero behavior change.
## Not a scene node: instance with `CrowdBowl.new()`, call `build(root)` once in
## `_ready()`, keep the instance around to call `set_intensity()` later.
...
## Reuses the crowd texture already on `root`'s flat back wall ("Stands_Far"),
## then hides that wall — the arc replaces it. To swap in a new crowd image,
## drop it on Stands_Far's Albedo texture slot; this picks it up automatically.
func build(root: Node3D) -> void:
	var crowd_tex: Texture2D = null
	var back := root.get_node_or_null("Stands_Far") as MeshInstance3D
	if back != null:
